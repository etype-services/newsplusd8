...
class Encoding {
   ...
   * @param string $text
   *   Any string.
   *   This function leaves UTF8 characters alone,
   *   while converting almost all non-UTF8 to UTF8.
   *   It assumes that the encoding of the original string
   *   is either Windows-1252 or ISO 8859-1.
   *   It may fail to convert characters to UTF-8
   *   if they fall into one of these scenarios:
   *   1) when any of these characters:   ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞß
   *    are followed by any of these:  ("group B")
   *                                    ¡¢£¤¥¦§¨©ª«¬­®¯°±²³´µ¶•¸¹º»¼½¾¿
   *   For example:   %ABREPRESENT%C9%BB. «REPRESENTÉ»
   *   The "«" (%AB) character will be converted,
   *   but the "É" followed by "»" (%C9%BB)
   ...
   *   Description.
   */
  public function toWin1252($text, $option = self::WITHOUT_ICONV) {
    if (is_array($text)) {
      foreach ($text as $k => $v) {
  ...
  public function utf8FixWin1252Chars($text) {
    // If you received an UTF-8 string
    // that was converted from Windows-1252 as it was ISO8859-1.
